...
![Job Hunter illustrated workflow: connect personal conditions to job requirements, explain the match, then verify and remember authorized actions](docs/media/demo.en.gif)
...
The host reasons using the Skill; `browser_actions.py` routes individual steps through policy and browsing checks, a BOSS DOM adapter and `webbridge_client.py`. `store.py` persists checkpoints, deduplication and receipts.
